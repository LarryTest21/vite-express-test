...
const emitSaved = (n: any) => {
  if (n === 1) {
    if (savedPost.value != undefined) {
      ...
      editor.commands.insertContent(postContent.value)
    ...
    axios.post("/api/user/refresh").then(() => {
      axios.post("/api/user/updateSavedPost/" + userID.value, { savedPost: null }).then((res) => {
...
<template>
  <div class="wrapper" ref="el">
    ...
    <transition name="savedModal">
      <Modal v-if="showModal" :class="{ save: modalMessage != '' }" class="modalComp" :modalLoadingMessage="modalMessage"
        :fontSize="'2rem'" :position="'absolute'" :backgroundOpacity="1" :modalSaved="modalSaved"
        @emitSaved="emitSaved" />
    </transition>
      ...
      <div class="editor">
        <div class="buttons-editor">
          <div class="post-title-wrapper">
            <div class="post-title"><label>Post Title</label>
            ...
            <div class="selection-wrapper">
              ...
              <div class="selects">
                <!-- <Multiselect class="multiselect" v-model="multiSelectSelected" :options="multiSelectOptions"
                  :multiple="true" :searchable="false" placeholder="Select main category">
                </Multiselect>
                <Multiselect class="multiselect" v-model="multiSelectSelected2" :options="multiSelectOptions2"
                  :multiple="true" :searchable="false" placeholder="Select sub category">
                </Multiselect> -->
                <MultiSelect :multiSelectOptions="mainCategory" :fontSize="'1.5rem'" :deleteAble="false"
                  @mainCategory="mainCategoryEmit" />
                <MultiSelect :multiSelectOptions="subCategory" :deleteAble="true" @subCategory="subCategoryEmit" />
              </div>
            </div>
          </div>
          <div class="buttons">
        ...
        <createPostSide :key="refreshCalendar" class="post-side" :userID="userID" :savedPost="savedPost"
          :showSavedPost="showSavedPost" :postTitle="postTitle" :postContent="postContent" @postSaved="postSaved"
          @interPost="interPost" @showPreview="showPreview" :emittedMainCategory="emittedMainCategory"
