...
const searchArray = ref(props.storedPosts) as any;
  ...
  if (newValue != "" || newValue != undefined) {
    ...
    if (searchValue.value !== "") {
      ...
      resultsTitle.value = searchArray.value.filter(
        (item:any) => item.postTitle.indexOf(searchValue.value) !== -1
      );
      resultsContent.value = searchArray.value.filter(
        (item:any) => item.postContent.indexOf(searchValue.value) !== -1
      );
      resultsAuthor.value = searchArray.value.filter(
        (item:any) => item.postAuthor.indexOf(searchValue.value) !== -1
      );
              ...
              <TransitionGroup name="results">
                <div class="results" v-for="result in results" :key="1">
                  ...
                  <div class="where-found-wrapper">
                    ...
                    <div
                      class="search-place"
                      v-if="result.whereFound1 !== undefined"
                    >
                      "{{ result.whereFound1 }}"
                    </div>
                    <div
                      class="search-place"
                      v-if="result.whereFound2 !== undefined"
                    >
                      "{{ result.whereFound2 }}"
                    </div>
                    <div
                      class="search-place"
                      v-if="result.whereFound3 !== undefined"
                    >
                      "{{ result.whereFound3 }}"
                    </div>
                  </div>
                  <div class="post-date">
                    {{
                      moment(new Date(result.postDate.toDate())).format(
                        "MMM DD, HH:mm"
                      )
                    }}
                  </div>
